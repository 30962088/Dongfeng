...
import com.media.dongfeng.view.SucaiTopView;
...
public class SucaiFragment extends Fragment {
		...
		if (content == null) {
			...
			if (content.isRead == false) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							NetDataSource.getInstance(getActivity()).ReadSucai(
									MainTabActivity.mUser, fid, 1);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (MainTabActivity.mUser != null) {
			mSucaiList = Utils.loadSucaiCidList(getActivity(),
					MainTabActivity.mUser);
		}
		...
		mListView.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				// + visibleItemCount) {
				// // Log.d("net",
				// "mListView onScroll loadMore firstVisibleItem="+firstVisibleItem+"  visibleItemCount="+visibleItemCount+"    totalItemCount="+totalItemCount);
				// // mGetDataTask.switchListViewMode(false);
				// // mGetDataTask.LoadMoreList(MainTabActivity.mUser);
				// }
				// }
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// SucaiHuodongDetailFragment fragment = new
				// SucaiHuodongDetailFragment(content, true);
				// transation.replace(R.id.sucai_container, fragment,
				// SucaiActivity.SUCAI_DETAIL_FRAGMENT);
		...
		mSearchListView = (RTPullListView) getView().findViewById(
				R.id.lvSearchSucai);
		mSearchListView
					...
					public void onRefresh() {
						...
						try {
							if (!TextUtils.isEmpty(mSearchText)) {
								...
								mGetDataTask.RefreshSearchList(
										MainTabActivity.mUser, mSearchText,
										true);
							}
						} catch (Exception e) {
						}
		...
		mSearchListView.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				// + visibleItemCount) {
				// // Log.d("net",
				// "mSearchListView onScroll loadMore firstVisibleItem="+firstVisibleItem+"  visibleItemCount="+visibleItemCount+"    totalItemCount="+totalItemCount);
				// mGetDataTask.switchListViewMode(true);
				// mGetDataTask.LoadMoreSearchList(MainTabActivity.mUser,
				// mSearchText);
				// }
				// }
			}
		...
		etSearchText
				.setOnEditorActionListener(new TextView.OnEditorActionListener() {
					...
					public boolean onEditorAction(TextView v, int actionId,
						...
						if (TextUtils.isEmpty(mSearchText)) {
							mGetDataTask.switchListViewMode(false);
							mGetDataTask.RefreshList(MainTabActivity.mUser,
									false);
						} else {
							mSearchText = mSearchText.replaceAll(" ", "-");
							mGetDataTask.switchListViewMode(true);
							mGetDataTask.RefreshSearchList(
									MainTabActivity.mUser, mSearchText, false);
						}
		...
		if (fid == 0) {
			getView().findViewById(R.id.search_field).setVisibility(
					View.VISIBLE);
		} else {
	...
	public void onPause() {
		...
		super.onPause();
		if (MainTabActivity.mUser != null) {
			Utils.saveSucaiCidList(getActivity(), MainTabActivity.mUser,
					mSucaiList);
		}
	...
	private class SucaiAdapter extends BaseAdapter {
		...
		private class SendMailTask extends AsyncTask<Void, Void, Boolean> {
			...
			protected Boolean doInBackground(Void... args) {
				try {
					if (NetDataSource.getInstance(getActivity()).sendMail(
							MainTabActivity.mUser, content.cfid, 1)) {
						return true;
					}
				} catch (ZhiDaoIOException e) {
				} catch (ZhiDaoApiException e) {
	...
	private class GetDataTask {
		...
		public void RefreshList(User user, boolean isShowPullDownView) {
			...
			try {
				mPage = 1;
				new GetDataTaskInternal().execute(new Object[] { user, mPage,
						null, true, isShowPullDownView });
		...
		public void LoadMoreList(User user) {
			...
			mIsListTaskFree = false;
			try {
				new GetDataTaskInternal().execute(new Object[] { user,
						mPage + 1, null, false, false });
			} catch (RejectedExecutionException e) {
				mIsListTaskFree = true;
			}
		}

		public void RefreshSearchList(User user, String keyword,
				boolean isShowPullDownView) {
			...
			try {
				mSearchPage = 1;
				new GetDataTaskInternal().execute(new Object[] { user,
						mSearchPage, keyword, true, isShowPullDownView });
			} catch (RejectedExecutionException e) {
				if (isShowPullDownView) {
					mSearchListView.onRefreshComplete();
				}
		...
		public void LoadMoreSearchList(User user, String keyword) {
			...
			try {
				new GetDataTaskInternal().execute(new Object[] { user,
						mSearchPage + 1, keyword, false, false });
			} catch (RejectedExecutionException e) {
				mIsSearchListTaskFree = true;
			}
		}

		private class GetDataTaskInternal extends
				AsyncTask<Object, Void, List<Content>> {
			...
			protected List<Content> doInBackground(Object... params) {
				...
				if (isRefresh && !isShowPullDownView) {
					SucaiFragment.this.getActivity().runOnUiThread(
							new Runnable() {
								@Override
								public void run() {
				...
				try {
					ContentList contentList = NetDataSource.getInstance(
							mContext).getContentsList(user, fid, 0,
							Constants.LIST_COUNT, page, keyword);
					if (contentList != null
							&& !contentList.mContentList.isEmpty()) {
				...
				if (isSearch) {
					if (isRefresh) { // refresh
						...
						if (result == null || result.isEmpty()) {
							// Log.d("net",
							// "GetDataTaskInternal onPostExecute isSearch refresh result is empty");
							mSearchListView.setSelection(0);
							mSearchAdapter.notifyDataChange(mSearchList);
						} else {
							// Log.d("net",
							// "GetDataTaskInternal onPostExecute isSearch refresh result not empty");
						...
						if (result != null && !result.isEmpty()) {
							// Log.d("net",
							// "GetDataTaskInternal onPostExecute isSearch loadmore result not empty");
							...
							// Log.d("net",
							// "GetDataTaskInternal onPostExecute isSearch loadmore result is empty");
						}
					}
					if (isShowPullDownView) {
						mSearchListView.onRefreshComplete();
					...
					if (isRefresh && result != null
							&& result.size() == Constants.LIST_COUNT) {
						isMore = true;
					} else if (!isRefresh
							&& result != null
							&& mList != null
							&& result.size() - mList.size() == Constants.LIST_COUNT) {
						isMore = true;
					} else {
						isMore = false;
					}
					...
					if (isRefresh) { // refresh
						...
						if (result == null || result.isEmpty()) {
							// Log.d("net",
							// "GetDataTaskInternal onPostExecute not isSearch refresh result is empty");
							mListView.setSelection(0);
							mAdapter.notifyDataChange(mList);
						} else {
							// Log.d("net",
							// "GetDataTaskInternal onPostExecute not isSearch refresh result not empty");
							mList.addAll(result);
							mListView.setSelection(0);
							mAdapter.notifyDataChange(mList);
						}
						...
						if (result != null && !result.isEmpty()) {
							// Log.d("net",
							// "GetDataTaskInternal onPostExecute not isSearch loadmore result not empty");
							...
							// Log.d("net",
							// "GetDataTaskInternal onPostExecute not isSearch loadmore result is empty");
						}
					}
					if (isShowPullDownView) {
						mListView.onRefreshComplete();
